...
export const DecisionTreeNavigator: React.FC<DecisionTreeNavigatorProps> = ({
  ...
  return (
    <div className="w-full space-y-4">
      {/* =========================================================================
          WIZARD HEADER: Pasos, Breadcrumbs y Botón de Reiniciar
          ========================================================================= */}
      <div className="bg-white border border-slate-200/90 rounded-2xl p-4 sm:p-5 shadow-xs">
        <div className="flex flex-wrap items-center justify-between gap-3">
          {/* Step Counter Pill */}
          <div className="flex items-center gap-2.5">
            <span className="w-8 h-8 rounded-xl bg-blue-600 text-white font-extrabold flex items-center justify-center text-sm shadow-2xs">
              {navPath.length + 1}
            </span>
            <div className="flex flex-col">
              ...
              <span className="text-sm font-bold text-slate-800">
                {currentNode ? currentNode.title : 'Selecciona una opción'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {navPath.length > 0 && (
              <button
                type="button"
                onClick={handleReset}
                className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-slate-500 hover:text-slate-800 hover:bg-slate-100 text-xs font-semibold transition-all cursor-pointer"
                title="Volver a la primera pregunta"
              >
        ...
        {/* Interactive Breadcrumbs Bar (Reducción de carga cognitiva con un clic para volver a cualquier nivel) */}
        {navPath.length > 0 && (
              ...
              <React.Fragment key={stepNode.id}>
                ...
                {idx === navPath.length - 1 ? (
                  ...
                  <button
                    ...
                    className="text-blue-700 hover:underline font-medium truncate max-w-xs cursor-pointer"
                  >
                    {stepNode.title}
                  </button>
                )}
      ...
      {/* =========================================================================
          WIZARD BODY: Muestra SOLAMENTE la pregunta o paso actual y sus opciones
          ========================================================================= */}
      {currentNode && currentNode.nodeType === 'step' ? (
        /* Vista de Paso de Tutorial */
        <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-xs space-y-5 animate-fadeIn">
          <div className="flex items-center gap-2">
        ...
        /* Vista de Contenido Final / Resolución */
        <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-xs space-y-6 animate-fadeIn">
          <div className="flex items-center gap-2">
        ...
        /* Vista de Pregunta / Selección de Opciones (Solo muestra la pregunta y respuestas actuales) */
        <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-xs animate-fadeIn">
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-2">
              <span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-lg bg-blue-50 text-blue-700 border border-blue-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 sm:gap-4">
            {currentOptions.map((node) => (
              <button
                ...
                className="bg-white border-2 border-slate-200 hover:border-blue-500 hover:bg-blue-50/40 active:bg-blue-50 active:border-blue-600 p-5 rounded-2xl flex items-center justify-between shadow-2xs hover:shadow-md transition-all active:scale-[0.98] group cursor-pointer text-left min-h-[76px]"
              >
                <div className="pr-3">
                  <span className="text-base sm:text-lg font-bold text-slate-800 group-hover:text-blue-700 leading-snug transition-colors">
                    {node.title}
                  </span>
                  {node.nodeType === 'step' && (
                    <span className="block text-xs font-semibold text-purple-600 mt-0.5">
                      Paso de tutorial
                    </span>
                  )}
                ...
                <div className="w-11 h-11 rounded-xl bg-slate-100 group-hover:bg-blue-600 group-hover:text-white text-slate-500 flex items-center justify-center shrink-0 transition-colors shadow-2xs">
          ...
          {currentOptions.length === 0 && (
            <div className="p-6 bg-slate-50 rounded-2xl border border-slate-200 text-center mt-4">
              ...
              <button
                type="button"
                onClick={handleGoBack}
                className="mt-3 px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-xl"
              >
